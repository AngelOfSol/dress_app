...
long ACRConnection::getLong(long index)
{
	long lBound = 0;
	CComVariant vItem;
...
ACRConnection::ptr make_connection(std::string ip)
{
	try
	{
		...
		unknown.CoCreateInstance(L"ComACRServer.Channel.1");

		if (unknown)
		{
			auto ret = std::make_unique<ACRConnection>();
			ret->m_channel = unknown;
			auto& channel = ret->m_channel;
			if (channel == nullptr)
			{
				return ACRConnection::ptr();
			}
