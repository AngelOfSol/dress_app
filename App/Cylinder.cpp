...
void Cylinder::buildInitial(void)
{
	...
	//this->reference_ = vec2d(0, this->initialSize_.y);
	this->states_.clear();
	this->moves_.clear();
...
void Cylinder::rebuildMesh()
{
	this->mesh_.clear();

	// construct the vertex array from the list of points we have
	
	this->mesh_.data().insert(this->mesh_.data().begin(), this->shape_.vertexes.rbegin(), this->shape_.vertexes.rend());
	this->mesh_.data().push_back(this->shape_.vertexes.back());
	
	this->mesh_.buildMesh();
}
